...
export const POST = withHandler<{ id: string; provider: string }>(
  async (
    ...
    if (upperProvider === 'MS_CLARITY') {
      return fail(400, 'NOT_SUPPORTED', 'Microsoft Clarity does not use OAuth authentication.');
    }
    ...
    if (!authUrl) {
      return fail(400, 'AUTH_URL_FAILED', 'Could not generate reconnect auth URL for provider');
    }
