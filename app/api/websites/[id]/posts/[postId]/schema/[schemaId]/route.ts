...
export const PATCH = withHandler(
  async (req: Request, { params }: { params: { id: string; postId: string; schemaId: string } }) => {
    const ctx = await requireOrgContext(req);
    await requireWebsiteAccess(ctx, params.id);

    const existing = await prisma.postSchema.findFirst({
      where: { id: params.schemaId, postId: params.postId, post: { websiteId: params.id } }
    });
    ...
    const updated = await prisma.postSchema.update({
...
export const DELETE = withHandler(
  async (req: Request, { params }: { params: { id: string; postId: string; schemaId: string } }) => {
    ...
    await prisma.postSchema.delete({
      where: { id: params.schemaId }
    });
